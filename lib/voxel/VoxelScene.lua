...
local SpriteBillboards = V.require("util/SpriteBillboards")
...
local function leanAngle()
  return VoxelScene.spriteLean or V.require("voxel/VoxelState").angle
end
...
local function shadowSignature(terrain, nbMesh, posed, cx, cy, vw, vh)
  ...
  put(vw); put(vh)
  put(math.floor((V.require("voxel/VoxelState").angle or 0) * 512))
...
local function castShadows(state, terrain, nbMesh, posed, cx, cy, vw, vh,
  ...
  pcall(function()
    local stageArena, stageY = V.require("battle/OverworldBattle").stage()
    if stageArena and stageArena.discs then
      V.require("stadium/StadiumStage").cast(ShadowMap, stageArena, stageY or 0)
    end
    V.require("stadium/Stadium").cast(ShadowMap)
  end)
...
function VoxelScene.render(state, w, h, vw, vh, paletteFor, eyes)
  ...
  if eyes then
    local okB, cards, tex, token = pcall(function()
      return V.require("battle/OverworldBattle").worldCards()
    end)
  ...
  if battleTex then
    ...
    if okB and cards then
      ...
      pcall(function()
        local stageArena, stageY = V.require("battle/OverworldBattle").stage()
        if stageArena and stageArena.discs then
          V.require("stadium/StadiumStage").draw(stageArena, stageY or 0)
        end
      ...
      local okA, fxTex, fxModel = pcall(function()
        return V.require("battle/OverworldBattle").worldAnim()
      end)
