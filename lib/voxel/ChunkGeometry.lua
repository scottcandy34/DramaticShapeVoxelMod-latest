...
function ChunkGometry.flatTopRow(run, ty)
...
function ChunkGometry.run(map, bodyOnly, masks, sink, waterSink)
  ...
  for ty = -r, th + r - 1 do
    for tx = -r, tw + r - 1 do
      ...
      elseif s then
        ...
        elseif run then
          local topTile = map:tileAt(tx, ChunkGometry.flatTopRow(run, ty))
...
-- caller reads.
function ChunkGometry.of(map, bodyOnly, masks, split)
  local sink = MeshSinks.newTableSink()
  local waterSink = split and MeshSinks.newTableSink() or nil
  ChunkGometry.run(map, bodyOnly, masks, sink, waterSink)
...
function ChunkGometry.build(map, bodyOnly, masks, split)
  local sink = MeshSinks.newSink()
  local waterSink = split and MeshSinks.newSink() or nil
  ChunkGometry.run(map, bodyOnly, masks, sink, waterSink)
