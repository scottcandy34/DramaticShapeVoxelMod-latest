...
SettingsMenu.CATEGORIES = {
  ...
  { id = "battles", label = "BATTLES..",
    summary = function() return V.require("battle/OverworldBattle").setting:valueLabel() end,
  ...
  { id = "vr", label = "VR..",
    summary = function() return V.require("vr/VR").setting:valueLabel() end,
...
local function isFull()
  ...
  return V.require("voxel/VoxelState").isFull(Pipelines.level("voxel"))
end
...
function SettingsMenu.rows(catId, game)
  ...
  if catId == SettingsMenu.ROOT then
    ...
    local ok, importRow = pcall(function()
      return V.require("stadium/rom/StadiumRomPick").row()
    end)
  ...
  elseif input:wasPressed("select") then
    ...
    if help and self.game.stack then
      self.game.stack:push(
        V.require("ui/settings/SettingsHelp").new(self.game, row.label, help))
    end
    return
